  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-40 glass-nav transition-all duration-200 no-print">
        <div className="w-full max-w-[1920px] mx-auto px-2.5 sm:px-4 md:px-6 lg:px-8 xl:px-10">
          <div className="flex items-center justify-between h-14 sm:h-16 gap-2">
            
            {/* Logo & Marca */}
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-cyan-500 flex items-center justify-center text-white shadow-md sm:shadow-lg shadow-blue-500/25 border border-white/30 dark:border-white/15 shrink-0">
                <Calendar className="w-4 h-4 sm:w-5 sm:h-5 drop-shadow-xs" />
              </div>
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 hidden xl:block font-medium">Control Centralizado de Horas y Capacitaciones</p>
              </div>
            </div>

            {/* Navegación por pestañas (Visible en pantallas medianas y grandes: md+) */}
            <nav className="hidden md:flex items-center gap-1 bg-slate-200/50 dark:bg-slate-900/60 p-1 rounded-2xl border border-slate-200/60 dark:border-white/5 backdrop-blur-md transition-all shrink-0">
              {tabs.map((tab) => {
                const Icon = tab.icon;
                ...
                return (
                  <button
                    ...
                    title={tab.label}
                    className={`flex items-center gap-1.5 lg:gap-2 px-2.5 lg:px-3.5 py-1.5 rounded-xl text-xs lg:text-sm font-medium transition-all duration-200 cursor-pointer ${
                      isActive
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              ...
              {/* Botón de búsqueda compacto para tablet y móvil */}
              <button
                onClick={onOpenSearch}
                className="xl:hidden p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-white/80 dark:hover:bg-slate-800/80 border border-slate-200/70 dark:border-white/10 shadow-glass-sm transition-all cursor-pointer backdrop-blur-md"
                title="Buscar (Ctrl + K)"
              >
              ...
              {isAdmin ? (
                <div 
                  className="flex items-center gap-1 sm:gap-1.5 bg-emerald-50/80 dark:bg-emerald-950/60 border border-emerald-300/60 dark:border-emerald-700/60 px-2 sm:px-2.5 py-1.5 rounded-xl text-emerald-800 dark:text-emerald-300 text-xs font-bold shadow-glass-sm backdrop-blur-md animate-in fade-in duration-150"
                ...
                >
                  ...
                  <button
                    ...
                    className="p-1 hover:bg-emerald-200/70 dark:hover:bg-emerald-900/80 rounded-md text-emerald-700 dark:text-emerald-300 transition-colors cursor-pointer"
                  >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-xl border border-slate-200/70 dark:border-white/10 bg-white/60 dark:bg-slate-800/60 hover:bg-blue-50/80 dark:hover:bg-blue-950/50 text-slate-600 dark:text-slate-300 hover:text-blue-700 dark:hover:text-blue-400 text-xs font-semibold transition-all shadow-glass-sm backdrop-blur-md cursor-pointer"
                >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 sm:gap-2 liquid-btn-primary px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-bold shadow-md transition-all transform active:scale-95 cursor-pointer shrink-0"
              >
                <Plus className="w-4 h-4 stroke-[2.5]" />
                <span className="hidden sm:inline">Nueva Cita</span>
                <span className="sm:hidden text-xs">Cita</span>
              </button>
            </div>
          </div>
        </div>
